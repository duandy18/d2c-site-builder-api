...
def test_site_builder_navigation_returns_registered_pages() -> None:
    ...
    assert "site_builder.dashboard" in root_by_code
    ...
    dashboard = root_by_code["site_builder.dashboard"]
    assert dashboard["title"] == "建站模板"
    assert dashboard["route_path"] == "/"
    assert dashboard["component_key"] == "site_builder.template_catalog"
